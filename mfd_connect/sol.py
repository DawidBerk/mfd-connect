...
class SolConnection(Connection):
    ...
    def _deactivate_sol_session(self, retry_count: int = 2) -> None:
        ...
        defunct ipmiutil process(es) if all attempts fail. A failed attempt's
        lingering ipmiutil process is force-killed before the next retry, to
        avoid leaving extra defunct process(es) behind for later cleanup.
    ...
    def _kill_defunct_ipmiutil_processes(self) -> None:
        ...
        Processes are matched by command name via 'ps -C <name>' (server-side filtering,
        instead of listing every process and filtering in Python) and only STAT='T'
        entries are killed. 'T' means stopped by job control signal, while healthy
        ipmiutil usually has 'S' or 'S+' (interruptible sleep, waiting for an event).

        Process listing uses 'ps -eo pid,stat,comm --no-headers -C <tool_name>' - a fixed,
        three-column format parsed entirely in Python. PopenSpawn executes commands
        directly (no shell). If no process matches, 'ps' returns a non-zero exit code
        with empty output, which is handled the same as finding zero defunct processes.

        Finding no defunct process is not treated as fatal: it simply means there's
        nothing to clean up here (e.g. the earlier failure wasn't caused by a stuck
        ipmiutil process in the first place, or there was no prior session at all).

        :raises SolException: if process listing fails or kill operation fails
        """
        try:
            ps_process = pexpect.popen_spawn.PopenSpawn(
                f"ps -eo pid,stat,comm --no-headers -C {self._ipmi_tool_name}", timeout=30
            )
